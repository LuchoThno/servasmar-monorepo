...
if (!clientId || !clientSecret || clientId.startsWith('your-') || clientSecret.startsWith('your-')) {
  console.error('Configura GOOGLE_CLIENT_ID y GOOGLE_CLIENT_SECRET en apps/web/.env.local antes de ejecutar este script.')
  process.exit(1)
}
...
oauth2Client
  ...
  .then(({ tokens }) => {
    if (!tokens.refresh_token) {
      console.log('Google no devolvio refresh_token. Revoca el acceso de la app y vuelve a ejecutar con prompt=consent.')
      console.log('Tokens recibidos:', tokens)
      return
    }

    console.log('Agrega este valor a apps/web/.env.local y al hosting de Next.js:')
